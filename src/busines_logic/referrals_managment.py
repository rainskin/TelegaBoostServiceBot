...
from core.db import transactions
from enums.transaction_type import TransactionType
from loader import bot


async def register_new_referral(inviter_id: int, referral_id: int):
    await users.add_referral(inviter_id, referral_id)
    await users.assign_inviter(referral_id, inviter_id)
...
async def add_referral_reward(inviter_id: int, deposit_amount: float, referral_id: int, payment_id: str):
    referral_deposit_reward_percent = await admin.get_referral_deposit_reward()
    referral_reward = await get_referral_reward(deposit_amount, referral_deposit_reward_percent)
    user_balance = await users.get_balance(inviter_id)
    ...
    await users.add_balance(inviter_id, referral_reward)
    await users.increment_referrals_reward(inviter_id, referral_reward)
