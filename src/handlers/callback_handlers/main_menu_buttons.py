...
async def _(query: types.CallbackQuery, state: FSMContext):
    ...
    print(len(order_ids))  # check amount of orders
    ...
    while order_ids:
        ...
        if len(order_ids) > 50:
            ...
            order_ids_part = await get_orders_without_tg_stars_orders(order_ids)
            if not order_ids_part:
                await query.message.answer(messages.no_active_orders[lang],
                                           reply_markup=navigation_kb.orders(lang,
                                                                             current_orders=current_orders).as_markup())
                return
        ...
        await get_order_statuses_text(user_id, lang, order_ids_part, current_orders=current_orders)

        await storage.set_data(key, current_orders=current_orders, order_ids=order_ids_part)
...
async def _(query: types.CallbackQuery, state: FSMContext):
    await query.answer('This action temporarily unavailable.')
    return
    user_id = query.from_user.id
    lang = await users.get_user_lang(user_id)
    key = StorageKey(bot_id=bot.id, chat_id=user_id, user_id=user_id)
    ...
    order_ids = await get_orders_without_tg_stars_orders(order_ids)
...
async def get_orders_without_tg_stars_orders(order_ids: list[str]):
    print('Filtering standard orders')
    for order_id in order_ids:
        try:
            order_item = await orders_queue.get(order_id)
            if order_item:
