...
from utils.Payment_methods.aaio import payment_statuses
...
async def _(query: types.CallbackQuery, state: FSMContext):
    ...
    await query.message.answer(messages.payment_by_card[lang].format(order_id=internal_order_id), reply_markup=kb.as_markup())
    ...
    if data and internal_order_id == data['internal_order_id']:
        ...
        if status != 'success':
            text = (f'{messages.current_payment_status[lang]}:\n\n'
                    f'{payment_statuses.current_payment_status_translated[status][lang]}')
