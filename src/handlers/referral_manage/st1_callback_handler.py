...
async def _(query: types.CallbackQuery, state: FSMContext):
    ...
    currency = 'RUB'
    reward_percent = await admin.get_referral_deposit_reward()
    referral_list = await users.get_all_referrals(user_id)
    referral_amount = len(referral_list) if referral_list else 0
    total_earned = await users.get_referrals_reward(user_id)
