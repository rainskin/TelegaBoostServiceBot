...
async def make_request(url: str, retries: int = 3, cooldown: int = 5, source_name: str = '') -> Any:
    for attempt in range(1, retries + 1):
        try:
            ...
            error_message = (
                f"❌ Непредвиденная ошибка при обработке запроса во время выполнения функции <b>{source_name}</b>\n"
                f"Ошибка: {str(e)}"
            )
            await bot.send_message(chat_id=config.ADMIN_ID, text=error_message)
            return None
...
async def get_available_categories():
    method = 'services'
    url = f'{BASE_URL}{method}&key={API_TOKEN}'
    services = await make_request(url, source_name='get_available_services')
...
async def get_services_by_category(category_name):
    method = 'services'
    url = f'{BASE_URL}{method}&key={API_TOKEN}'
    categories = await make_request(url, source_name='get_services_by_category')
...
async def get_service(service_id: int):
    method = 'services'
    url = f'{BASE_URL}{method}&key={API_TOKEN}'
    services = await make_request(url, source_name='get_service')
...
async def get_order_statuses(order_ids: List[str]) -> Dict[str, dict]:
    ...
    orders: Dict[str, dict] = await make_request(url, source_name='get_order_statuses')
...
async def create_new_order(service_id: str, link: str, quantity: int) -> int | None:
    ...
    response = await make_request(url, source_name='create_new_order')
